...
handle_control_line(const char *line, int *ac_status, struct battery_status *batteries)
{
    ...
    value_p = strchr(line, '=');
    if (!value_p) {
        printk(KERN_ERR "fake_battery: malformed line (missing '='): %s\n", line);
        return -EINVAL;
    }

    value_p++;  // move past '='
    value_p = skip_spaces(value_p);
    ...
    if(prefixed(line, "capacity")) {
        ...
        *ac_status = value;
        printk(KERN_INFO "fake_battery: received 'charging = %ld', updating ac_status = %d\n", value, *ac_status);
        power_supply_changed(supplies[1]); // notify system that AC0 changed
    } else {
        return -EINVAL;
    }
...
{
    printk(KERN_INFO "fake_battery: handle_charge_changes(ac_status = %d, capacity = %d)\n",
       ac_status, battery->capacity);
    printk(KERN_INFO "fake_battery: -> status set to %d\n", battery->status);
    ...
    // 🛠 FIXED: compare battery->capacity, not battery->capacity_level
...
control_device_write(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
    ...
    printk(KERN_INFO "fake_battery: write completed, ac_status = %d\n", ac_status);
        ...
        union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_ONLINE:
        ...
        // 👇 This is the optional debug line:
        printk(KERN_INFO "fake_battery: AC0 ONLINE queried, returning %d\n", ac_status);
